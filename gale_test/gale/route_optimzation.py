...
class CreateTotalTimeCallback(object):
  ...
  def TotalTime(self, from_node, to_node):
    print self.service_time_callback(from_node, to_node) + self.dist_callback(from_node, to_node) / self.speed
...
def create_data_array():
  ...
  with open('output_new.csv') as csvfile:
    ...
    for row in reader:
#         address = row['ADDRESS'] + str(" ") + row['CITY'] + str(" ") + row['STATE'] + str(" ") + row['PIN']
#   
...
#              19, 1, 24, 8, 12, 4, 8, 24, 24, 2, 20, 15, 2, 14, 9]
  ...
  print sum(demands), len(start_times)
  import pdb
  pdb.set_trace()
